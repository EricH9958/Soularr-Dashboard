...
The Soularr Dashboard is now available as a Docker container, making it easy to deploy alongside your Soularr installation.

### How to Pull the Docker Image

You can pull the prebuilt Docker image from GitHub Container Registry: `docker pull ghcr.io/erich9958/soularr-dashboard:latest`
...
This setup assumes that your Soularr container is already configured to write logs to `/opt/Soularr_docker/logs` and uses `/opt/Soularr_docker/config` for its configuration files.
...
|----------------|---------------------------------|------------------------|
| LOG_PATH       | Path to the Soularr log file   | `/logs/soularr.log`    |
| FAILURE_PATH   | Path to the failure list file  | `/data/failure_list.txt` |
| TZ             | Timezone for the container     | `UTC`                  |

### Running Manually

If you prefer not to use Docker Compose, you can run the container manually using:

docker run -d \
  --name soularr-dashboard \
  --network host \
  -v /opt/Soularr_docker/logs:/logs \
  -v /opt/Soularr_docker/config:/data \
  -e LOG_PATH=/logs/soularr.log \
  -e FAILURE_PATH=/data/failure_list.txt \
  -e TZ=America/Los_Angeles \
  ghcr.io/erich9958/soularr-dashboard:latest

This will start the dashboard container and allow it to monitor logs and failures from your Soularr instance.
=======
# Soularr Dashboard
